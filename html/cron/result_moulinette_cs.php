...
function manage_results($number) {
	...
	$destination = "$boinc/sample_results/managed/$name/$name.dat.gz";
...
//===============================================================================
//get lock
$lockfile = "/tmp/result_moulinette_cs.lock";
$res = @stat($lockfile);
if($res===false) {
...
echo "$stamp: starting rsals results moulinette cs\n";
$time_start = microtime(true);


$numbers = BoincNumber_cs::enum("(status=1 OR status=2) AND q_start<>0 AND q_end<>0");
$count = count($numbers);
