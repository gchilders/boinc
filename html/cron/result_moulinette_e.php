...
//===============================================================================
//get lock
$lockfile = "/tmp/result_moulinette_e.lock";
$res = @stat($lockfile);
if($res===false) {
    $lock = fopen($lockfile,"wb");
} else {
    echo "moulinette_e already running\n";
    exit(0);
}
db_init();
$now=time();
...
echo "$stamp: starting rsals results moulinette e\n";
$time_start = microtime(true);


$numbers = BoincNumber_e::enum("(status=1 OR status=2) AND q_start<>0 AND q_end<>0");
$count = count($numbers);
