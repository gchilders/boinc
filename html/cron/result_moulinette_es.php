...
//===============================================================================
//get lock
$lockfile = "/tmp/result_moulinette_es.lock";
$res = @stat($lockfile);
if($res===false) {
    $lock = fopen($lockfile,"wb");
} else {
    echo "moulinette_es already running\n";
    exit(0);
}
db_init();
$now=time();
...
$numbers = BoincNumber_es::enum("(status=1 OR status=2) AND q_start<>0 AND q_end<>0");
$count = count($numbers);
