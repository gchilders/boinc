...
function find_files_in($folder, $wildcard) {
	$results = Array();
	$cmd="find $folder -maxdepth 1 -name '$wildcard' | head -5000";
...
function manage_results($number) {
	...
	$name = $number->name;
	$wuid = $number->id;
	$folder = "$boinc/sample_results/mine/$name";
	//--------------------
	...
	$destination = "$boinc/sample_results/mine/$name/$name.dat.gz";
...
//===============================================================================
//get lock
$lockfile = "/tmp/result_moulinette_f.lock";
$res = @stat($lockfile);
if($res===false) {
    $lock = fopen($lockfile,"wb");
} else {
    echo "moulinette_f already running\n";
    exit(0);
}
db_init();
$now=time();
...
echo "$stamp: starting rsals results moulinette fs\n";
$time_start = microtime(true);


$numbers = BoincNumber_f::enum("(status=1 OR status=2) AND q_start<>0 AND q_end<>0");
$count = count($numbers);
