...
function generate_work($number, $q_first, $q_interval, $num_wus) {
	...
	$numname=trim($number->name);
        $polyname = sprintf("%s.poly",trim($number->name));
	$results=Array();
	...
	for($i=0; $i<$num_wus; $i++) {
		...
		if ($wupriority == 0) $wupriority = $wuid;
                $wuname = sprintf("%s_%d",trim($number->name),$q_first/1000);

		$command ="cd $boinc && bin/create_work ";
		$command.="--batch $wuid ";
		$command.="--priority $wupriority ";
		$command.="--appname lasieved ";
...
// MAIN ENTRY POINT
//===============================================================================
$lockfile = "/tmp/workunit_moulinette.lock";              
$res = @stat($lockfile);
if($res===false) {           
...
$numbers = BoincNumber::enum("status=1 AND q_start<>0 AND q_end<>0 AND q_last<q_end");
$count = count($numbers);
if($count>0) {
