...
*/
	...
	for($i=0; $i<$num_wus; $i++) {
		...
		$creditvalue=700 * 2**($totalbits - 27);
		if ($totalbits == 31) $app = "cudasieve";
		elseif ($totalbits == 30) $app = "cudasieve";
		else $app = "cudasieve";
...
function manage_creation($number,$rerate) {
	...
	if($count_unsent < $rerate) {
		...
		if($required>0) {
			...
			// $wus_required = $required; // Make all
...
$lockfile = "/tmp/workunit_moulinette_cs.lock";              
$res = @stat($lockfile);
if($res===false) {           
    $lock = fopen($lockfile,"wb");
} else {
    echo "moulinette_cs already running\n";
    exit(0);        
}
db_init();
$now=time();
...
$numbers = BoincNumber_cs::enum("status=1 AND q_start<>0 AND q_end<>0 AND q_last<q_end");
$count = count($numbers);
if($count>0) {
