...
function generate_work($number, $q_first, $q_interval, $num_wus) {
	...
	for($i=0; $i<$num_wus; $i++) {
		...
		$command.="--rsc_memory_bound 1100000000 ";
		$command.="--credit 50 ";
		$command.="$polyname";
		$results=Array();
...
$lockfile = "/tmp/workunit_moulinette_fs.lock";              
$res = @stat($lockfile);
if($res===false) {           
...
$numbers = BoincNumber_fs::enum("status=1 AND q_start<>0 AND q_end<>0 AND q_last<q_end");
$count = count($numbers);
if($count>0) {
