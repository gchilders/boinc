...
function calc_table($id,$item) {
	...
	if($item->q_end != 0 && $item->q_start!=0) {
		$total = ($item->q_end - $item->q_start) / 16000; //should be Q_PER_WU
		$done = ($item->q_last - $item->q_start) / 16000;
		$percent = $done / $total;
	...
	echo "<tr><td>Generated Q range</td><TD align='left'>$range $percent</td></tr>";
        if ($item->results_received > 9) {
                $needed_rels = 27500000 * pow(2,$item->primebits - 28);
                $rec_last = $needed_rels / $item->globresult_relations * $item->results_received * 16000 + $item->q_start; 
...
$result = _mysql_query("SELECT * FROM number");
...
function update_if_changed($form, $id, $old_v, $sql_field, $check_larger=false, $compare=null) {
    ...
    if( $new_v != $old_v && $new_v!='') {
        $cmd =  "UPDATE number SET $sql_field='$new_v' WHERE id=$id";
...
if( !empty($_POST) ) {
	...
	for($j=1;$j<=$Nrow;$j++){  // test/update each row in DB
        ...
        if( isset($_POST[$field]) && $_POST[$field]=='DELETE' ) {
            $cmd =  "DELETE FROM number WHERE id=$id";
            $commands .= "<P><pre>$cmd</pre>\n";
            _mysql_query("SET SQL_MODE = ''");
            _mysql_query($cmd);
		...
		$commands .= 	update_if_changed("status",			$id, $item->status			, "status"	, false, $item->status); //new > old!
    ...
    if( isset($_POST['add_number']) && $_POST['add_number'] ) {
        ...
        if( empty($name) || empty($project) ) {
            ...
            $now=time();
            $cmd =  "INSERT INTO number (name,create_time,project) ".
...
admin_page_head("Manage Project Numbers");
...
$q="SELECT * FROM number WHERE status=0 ORDER BY id";
...
$q="SELECT * FROM number WHERE status=1 ORDER BY id";
$result = _mysql_query($q);
if(_mysql_num_rows($result)>0) {
	display_result($result,true);
} else {
...
$q="SELECT * FROM number WHERE status=2 ORDER BY id";
$result = _mysql_query($q);
if(_mysql_num_rows($result)>0) {
...
$q="SELECT * FROM number WHERE status=3 ORDER BY id";
$result = _mysql_query($q);
if(_mysql_num_rows($result)>0) {
	display_result($result,false);
} else {
...
echo"<P><hr><hr>
     <h2>COMPLETED</h2>\n
     <p><a href=\"manage_crunching_full.php\">Results over one year old</a></p>\n";

$q="SELECT * FROM number WHERE status=4 and create_time >= unix_timestamp(date_sub(now(), interval 1 year)) ORDER BY id";
$result = _mysql_query($q);
if(_mysql_num_rows($result)>0) {
