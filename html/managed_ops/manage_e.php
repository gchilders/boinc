...
$result = _mysql_query("SELECT * FROM number_e");
...
function update_if_changed($form, $id, $old_v, $sql_field, $check_larger=false, $compare=null) {
    ...
    if( $new_v != $old_v && $new_v!='') {
        $cmd =  "UPDATE number_e SET $sql_field='$new_v' WHERE id=$id";
...
if( !empty($_POST) ) {
	...
	for($j=1;$j<=$Nrow;$j++){  // test/update each row in DB
        ...
        if( isset($_POST[$field]) && $_POST[$field]=='DELETE' ) {
            $cmd =  "DELETE FROM number_e WHERE id=$id";
            $commands .= "<P><pre>$cmd</pre>\n";
           _mysql_query("SET SQL_MODE = ''");
           _mysql_query($cmd);
    ...
    if( isset($_POST['add_number']) && $_POST['add_number'] ) {
        ...
        if( empty($name) || empty($project) ) {
            ...
            $now=time();
            $cmd =  "INSERT INTO number_e (name,create_time,project) ".
...
admin_page_head("Manage lasievee Project Numbers");
...
// echo "<h3><a href=\"http://escatter11.fullerton.edu/nfs_data_e\">Data directory</a></h3>";
echo $commands;

echo"<P>
     <h2>QUEUED FOR SIEVING</h2>\n";

$q="SELECT * FROM number_e WHERE status=0 ORDER BY id";
...
$q="SELECT * FROM number_e WHERE status=1 ORDER BY id";
$result = _mysql_query($q);
if(_mysql_num_rows($result)>0) {
	display_result($result,true);
} else {
...
$q="SELECT * FROM number_e WHERE status=2 ORDER BY id";
$result = _mysql_query($q);
if(_mysql_num_rows($result)>0) {
...
$q="SELECT * FROM number_e WHERE status=3 ORDER BY id";
$result = _mysql_query($q);
if(_mysql_num_rows($result)>0) {
	display_result($result,false);
} else {
...
$q="SELECT * FROM number_e WHERE status=4 ORDER BY id";
$result = _mysql_query($q);
if(_mysql_num_rows($result)>0) {
