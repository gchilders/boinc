...
function calc_table($id,$item) {
	...
	if($item->q_end != 0 && $item->q_start!=0) {
		$total = ($item->q_end - $item->q_start) / 1000; //should be Q_PER_WU
		$done = ($item->q_last - $item->q_start) / 1000;
		$percent = $done / $total;
	...
	echo "<tr><td>Generated Q range</td><TD align='left'>$range $percent</td></tr>";
        if ($item->results_received > 9) {
                ...
                else if ($item->primebits == 34) { $needed_rels = 1200000000*1.1; }
                else { $needed_rels = 10000000; }
                $rec_last = $needed_rels / $item->globresult_relations * $item->results_received * 1000 + $item->q_start; 
...
$result = _mysql_query("SELECT * FROM number_fs");
...
function update_if_changed($form, $id, $old_v, $sql_field, $check_larger=false, $compare=null) {
    ...
    if( $new_v != $old_v && $new_v!='') {
        $cmd =  "UPDATE number_fs SET $sql_field='$new_v' WHERE id=$id";
...
if( !empty($_POST) ) {
	...
	for($j=1;$j<=$Nrow;$j++){  // test/update each row in DB
        ...
        if( isset($_POST[$field]) && $_POST[$field]=='DELETE' ) {
            $cmd =  "DELETE FROM number_fs WHERE id=$id";
            $commands .= "<P><pre>$cmd</pre>\n";
           _mysql_query("SET SQL_MODE = ''");
           _mysql_query($cmd);
    ...
    if( isset($_POST['add_number']) && $_POST['add_number'] ) {
        ...
        if( empty($name) || empty($project) ) {
            ...
            $now=time();
            $cmd =  "INSERT INTO number_fs (name,create_time,project) ".
...
admin_page_head("Manage lasievef_small Project Numbers");
...
$q="SELECT * FROM number_fs WHERE status=0 ORDER BY id";
...
$q="SELECT * FROM number_fs WHERE status=1 ORDER BY id";
$result = _mysql_query($q);
if(_mysql_num_rows($result)>0) {
	display_result($result,true);
} else {
...
$q="SELECT * FROM number_fs WHERE status=2 ORDER BY id";
$result = _mysql_query($q);
if(_mysql_num_rows($result)>0) {
...
$q="SELECT * FROM number_fs WHERE status=3 ORDER BY id";
$result = _mysql_query($q);
if(_mysql_num_rows($result)>0) {
	display_result($result,false);
} else {
...
$q="SELECT * FROM number_fs WHERE status=4 ORDER BY id";
$result = _mysql_query($q);
if(_mysql_num_rows($result)>0) {
