...
function calc_table($id,$item) {
	...
	if($item->q_end != 0 && $item->q_start!=0) {
		$total = ($item->q_end - $item->q_start) / 2000; //should be Q_PER_WU
		$done = ($item->q_last - $item->q_start) / 2000;
		$percent = $done / $total;
	...
	echo "<tr><td>Generated Q range</td><TD align='left'>$range $percent</td></tr>";
        if ($item->results_received > 9) {
                ...
                else if ($item->primebits == 33) { $needed_rels = 1200000000; }
                else if ($item->primebits == 34) { $needed_rels = 2100000000; }
                else { $needed_rels = 10000000; }
                $rec_last = $needed_rels / $item->globresult_relations * $item->results_received * 2000 + $item->q_start; 
    ...
    if ($v > 4294964000) {
        echo "<tr><td>WARNING</td><TD align='left'>Special Q above 2^32 will not appear in relations file. Special processing is needed.</TD></tr>\n";
    }
//update button
...
echo "<form action='$self' method='POST'>\n";
        ...
        $filedel = "/home/boincadm/projects/nfs/sample_results/mine/".$item->name;
...
$result = _mysql_query("SELECT * FROM number_f");
...
function update_if_changed($form, $id, $old_v, $sql_field, $check_larger=false, $compare=null) {
    ...
    if( $new_v != $old_v && $new_v!='') {
        $cmd =  "UPDATE number_f SET $sql_field='$new_v' WHERE id=$id";
...
if( !empty($_POST) ) {
	...
	for($j=1;$j<=$Nrow;$j++){  // test/update each row in DB
        ...
        if( isset($_POST[$field]) && $_POST[$field]=='DELETE' ) {
            $cmd =  "DELETE FROM number_f WHERE id=$id";
            $commands .= "<P><pre>$cmd</pre>\n";
           _mysql_query("SET SQL_MODE = ''");
           _mysql_query($cmd);
    ...
    if( isset($_POST['add_number']) && $_POST['add_number'] ) {
        ...
        if( empty($name) || empty($project) ) {
            ...
            $now=time();
            $cmd =  "INSERT INTO number_f (name,create_time,project) ".
...
admin_page_head("Manage lasieve5f Project Numbers");
...
$q="SELECT * FROM number_f WHERE status=0 ORDER BY id";
...
$q="SELECT * FROM number_f WHERE status=1 ORDER BY id";
$result = _mysql_query($q);
if(_mysql_num_rows($result)>0) {
	display_result($result,true);
} else {
...
$q="SELECT * FROM number_f WHERE status=2 ORDER BY id";
$result = _mysql_query($q);
if(_mysql_num_rows($result)>0) {
...
$q="SELECT * FROM number_f WHERE status=3 ORDER BY id";
$result = _mysql_query($q);
if(_mysql_num_rows($result)>0) {
	display_result($result,false);
} else {
...
$q="SELECT * FROM number_f WHERE status=4 ORDER BY id";
$result = _mysql_query($q);
if(_mysql_num_rows($result)>0) {
