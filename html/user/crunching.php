	...
	foreach ($numbers as $number) {
		...
		if($details) {
			if($number->q_end != 0 && $number->q_start != 0) {
				...
				$numwu = ($number->q_end - $number->q_start)/16000;
				$numunc = ($number->q_end - $number->q_last)/16000;
...
page_head(tra("Detailed status of lasieved"));
echo tra("%1 currently is sieving the following numbers using the lasieved app. When you participate in %1, work for one or more of these numbers will be assigned to your computer. The parameters defining the number will be downloaded to your computer, and the computed 'relations' needed for the post processing step will be uploaded to the main server. This happens automatically; you don't have to do anything. See the bottom of the page for the meaning of each column.", PROJECT)."<br>";



echo "<h2>Now sieving</h2>";
echo "<p>These numbers are being distributed to the clients right now. You can notice them because the workunits you get starts with these names. We need ~55-60M relations for 29-bits numbers, 110+M for 30 bits numbers, and 210+M for 31 bits numbers.</p>";

$numbers = BoincNumber::enum("status=1");
display_result($xml,$numbers,"working",true);
$res = _mysql_query("SELECT SUM(globresult_size) AS size FROM number WHERE status=1");
...
$numbers = BoincNumber::enum("status=0");
...
$numbers = BoincNumber::enum("status=2");
display_result($xml,$numbers,"pp_queued",true);
$res = _mysql_query("SELECT SUM(globresult_size) AS size FROM number WHERE status=2");
if($res!==false && _mysql_num_rows($res)==1) {
	$obj=_mysql_fetch_object($res);
	$totalbytes=((int)($obj->size/10000000))/100;
	_mysql_free_result($res);
...
echo "<p><strong>Q range: </strong>Sieving range, indicating the total number of workunits, each one managing 16k Q values</p>";
