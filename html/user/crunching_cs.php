...
function display_result($xml, $numbers, $type, $details=false) {
	...
	foreach ($numbers as $number) {
		...
		if($details) {
			if ($number->id < 20) $q_per_wu=100000; else $q_per_wu=250000;
...
$numbers = BoincNumber_cs::enum("status=1");
display_result($xml,$numbers,"working",true);
$res = _mysql_query("SELECT SUM(globresult_size) AS size FROM number_cs WHERE status=1");
...
$numbers = BoincNumber_cs::enum("status=0");
...
$numbers = BoincNumber_cs::enum("status=2");
display_result($xml,$numbers,"pp_queued",true);
$res = _mysql_query("SELECT SUM(globresult_size) AS size FROM number_cs WHERE status=2");
if($res!==false && _mysql_num_rows($res)==1) {
	$obj=_mysql_fetch_object($res);
	$totalbytes=((int)($obj->size/10000000))/100;
	_mysql_free_result($res);
...
$numbers = BoincNumber_cs::enum("status=3");
...
$numbers = BoincNumber_cs::enum("status=4");
