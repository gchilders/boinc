	...
	foreach ($numbers as $number) {
		...
		if($details) {
			if($number->q_end != 0 && $number->q_start != 0) {
				...
				$numwu = ($number->q_end - $number->q_start)/1000;
				$numunc = ($number->q_end - $number->q_last)/1000;
...
$numbers = BoincNumber_fs::enum("status=1");
display_result($xml,$numbers,"working",true);
$res = _mysql_query("SELECT SUM(globresult_size) AS size FROM number_fs WHERE status=1");
...
$numbers = BoincNumber_fs::enum("status=0");
...
$numbers = BoincNumber_fs::enum("status=2");
display_result($xml,$numbers,"pp_queued",true);
$res = _mysql_query("SELECT SUM(globresult_size) AS size FROM number_fs WHERE status=2");
if($res!==false && _mysql_num_rows($res)==1) {
	$obj=_mysql_fetch_object($res);
	$totalbytes=((int)($obj->size/10000000))/100;
	_mysql_free_result($res);
...
$numbers = BoincNumber_fs::enum("status=3");
...
$numbers = BoincNumber_fs::enum("status=4");
