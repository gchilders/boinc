...
$numbers = BoincNumber_f::enum("status=1");
display_result($xml,$numbers,"working",true);
$res = _mysql_query("SELECT SUM(globresult_size) AS size FROM number_f WHERE status=1");
...
$numbers = BoincNumber_f::enum("status=0");
...
$numbers = BoincNumber_f::enum("status=2");
display_result($xml,$numbers,"pp_queued",true);
$res = _mysql_query("SELECT SUM(globresult_size) AS size FROM number_f WHERE status=2");
if($res!==false && _mysql_num_rows($res)==1) {
	$obj=_mysql_fetch_object($res);
	$totalbytes=((int)($obj->size/10000000))/100;
	_mysql_free_result($res);
...
$numbers = BoincNumber_f::enum("status=3");
display_result($xml,$numbers,"post_processing",true);

// Remove below to remove completed numbers
echo "<h2>Completed</h2>";
echo "<p>These numbers have been successfully factored</p>";
$numbers = BoincNumber_f::enum("status=4");
